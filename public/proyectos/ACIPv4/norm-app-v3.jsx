...
const AREAS = [
{ slug: "vivienda-unifamiliar", num: "01", title: "Viviendas unifamiliares de obra nueva", img: R.ra1,
...
{ slug: "reformas", num: "02", title: "Reformas integrales premium", img: R.ra2,
...
const INVERSORES = { slug: "inversores", num: "03", title: "Inversores y promociones", img: R.ra3,
        ...
        <div className="nav-left">
          <div className={`nav-drop ${openDrop ? 'open' : ''}`} onMouseEnter={() => setOpenDrop(true)} onMouseLeave={() => setOpenDrop(false)}>
            ...
            <div className="nav-drop-panel">
              {AREAS.map((a) =>
              <a key={a.slug} className="nav-drop-item" onClick={() => {navigate("proyectos", a.slug);setOpenDrop(false);}}>
      ...
      <div className={`mobile-menu ${openMobile ? 'open' : ''}`}>
        ...
        <div className={`mob-acc ${openAcc ? 'open' : ''}`}>
          ...
          <div className="mob-acc-body">
            {AREAS.map((a) =>
            <a key={a.slug} onClick={() => {navigate("proyectos", a.slug);setOpenMobile(false);}}>{a.title}</a>
            )}
  ...
  return (
    <section className="hero hero-top">
      <div className="hero-media">
        ...
        <div key={i} className={`hero-slide ${i === idx ? 'active' : ''}`}><img src={src} alt="" /></div>
        )}
      </div>
      ...
      <div className="philosophy-inner">
        ...
        <div>
          ...
          <div className="philosophy-stats">
            ...
            <div key={l} className="philosophy-stat"><div className="n">{n}</div><div className="l">{l}</div></div>
            )}
          </div>
        </div>
      </div>
  ...
  return (
    <section className="areas" id="areas">
      ...
      <div className="areas-grid">
        {AREAS.map((a) =>
        <article key={a.slug} className="area" onClick={() => navigate("proyectos", a.slug)}>
...
// ============= PROCESO HORIZONTAL (estilo IN RESIDENCE) =============
function Proceso() {
  ...
  useEffect(() => {
    ...
    const s = { x: 0, max: 0, dragging: false, startX: 0, startScroll: 0, vx: 0, lastX: 0, lastT: 0, raf: 0 };
    const computeMax = () => {s.max = Math.max(0, track.scrollWidth - rail.clientWidth);};
    computeMax();
    const ro = new ResizeObserver(computeMax);
    ro.observe(track);ro.observe(rail);
    const setX = (x) => {
      s.x = Math.max(-60, Math.min(s.max + 60, x));
      track.style.transform = `translate3d(${-s.x}px, 0, 0)`;
    };
    const tick = () => {
      if (s.dragging) {s.raf = 0;return;}
      s.x += s.vx;
      if (s.x < 0) {s.x -= s.x * 0.2;s.vx *= 0.5;} else
      if (s.x > s.max) {s.x -= (s.x - s.max) * 0.2;s.vx *= 0.5;}
      s.vx *= 0.93;
      track.style.transform = `translate3d(${-s.x}px, 0, 0)`;
      if (Math.abs(s.vx) > 0.15 || s.x < 0 || s.x > s.max) s.raf = requestAnimationFrame(tick);else
      s.raf = 0;
    };
    const startTick = () => {if (!s.raf) s.raf = requestAnimationFrame(tick);};
    const getX = (e) => e.touches ? e.touches[0].pageX : e.pageX;
    const onDown = (e) => {
      s.dragging = true;
      const x = getX(e);
      s.startX = x;s.lastX = x;s.lastT = performance.now();
      s.startScroll = s.x;s.vx = 0;
    ...
    const onMove = (e) => {
      ...
      s.vx = (s.lastX - x) / dt * 16;
      s.lastX = x;s.lastT = now;
      setX(s.startScroll - (x - s.startX));
    };
    const onUp = () => {
      if (!s.dragging) return;
      s.dragging = false;
  ...
  return (
    <section className="proceso-h" id="proceso">
      <div className="proceso-h-head">
        <div>
          <span className="meta proceso-eyebrow">El proceso ACIP</span>
          <h2>Un proceso más claro. <span className="it">Una ejecución más coherente.</span></h2>
        </div>
        <p className="proceso-lede">La coordinación entre arquitectura y construcción nos permite desarrollar cada proyecto con una visión integrada, cuidada y controlada desde el inicio.</p>
      </div>
      <div className="proceso-h-rail" ref={railRef}>
        <div className="proceso-h-track" ref={trackRef}>
          {PROCESO.map((p) =>
          <article key={p.num} className="proceso-h-card">
              <div className="proceso-h-img"><img src={p.img} alt="" draggable="false" /></div>
              <div className="proceso-h-meta">
                <span className="proceso-h-num">{p.num}</span>
                <span className="proceso-h-tag">Fase</span>
              </div>
              <h3>{p.title}</h3>
              <p>{p.desc}</p>
            </article>
      ...
      <div className="proceso-h-hint" aria-hidden="true">
        <span>Arrastra para ver más</span>
        <svg width="48" height="10" viewBox="0 0 48 10" fill="none" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"><path d="M0 5h42m-5-4 5 4-5 4" /></svg>
      </div>
    </section>);
}
  ...
  return (
    <section className="journal" id="journal-home">
      <div className="section-head">
        ...
        <button className="meta-link" onClick={() => navigate("journal")}>Todas las entradas →</button>
      </div>
...
function CTAEmocional({ navigate }) {
  return (
    <section className="cta-emo" id="contacto">
      <img src={R['r-vision']} alt="" />
      <div className="cta-emo-inner">
        <h2>Cada vivienda comienza entendiendo <span className="it">cómo quiere vivir quien la habita.</span></h2>
        <p>Trabajamos proyectos residenciales de forma cercana, coordinada y completamente personalizada.</p>
        <div className="cta-emo-actions">
          <a className="btn-primary" onClick={() => navigate("contacto")}>Hablar sobre su proyecto</a>
          <a className="btn-ghost" onClick={() => navigate("home", "estudio")}>Conocer ACIP</a>
        </div>
      </div>
    </section>);
}
    ...
    <main>
      <section className="page-head">
        <div className="page-head-inner">
          ...
          <p className="lead">Reflexiones y proyectos: entradas de estudio sobre el oficio, materiales, presupuestos y decisiones de proyecto.</p>
        </div>
      </section>
    ...
    <main>
      <section className="inv-hero">
        ...
        <div className="inv-hero-inner">
          <div className="crumb light"><a onClick={() => navigate("home")}>ACIP</a><span>/</span><span>Inversores</span></div>
          <h1>Inversores y <span className="it">promociones.</span></h1>
          <p>{INVERSORES.desc}</p>
        </div>
      </section>
      ...
      <div className="footer-grid">
        ...
        <div className="footer-col">
          ...
          <ul>
            ...
            <li><a onClick={() => navigate("proyectos", "inversores")}>Inversores y promociones</a></li>
        ...
        <div className="footer-col">
          ...
          <ul>
            ...
            <li><a onClick={() => navigate("inversores")}>Inversores y promociones</a></li>
