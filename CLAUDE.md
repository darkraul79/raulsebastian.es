# raulsebastian.es — Portfolio personal

> SPA de portfolio personal. Laravel 12 + Vue 3 + Inertia + Tailwind CSS v4.
> Sin base de datos. Contenido via archivos `.md` en `resources/content/`.
...
- **league/commonmark** — v2
- **spatie/yaml-front-matter** — v2

## Node.js

- Usar Node v22 (disponible en Herd). El sistema tiene Node v18 por defecto.
- Para build: `export PATH="/Users/raulsebastian/Library/Application Support/Herd/config/nvm/versions/node/v22.21.1/bin:$PATH" && npm run build`
- O simplemente: `npm run dev` desde Herd/terminal con Node v22 activo.

## Comandos habituales

```bash
npm run dev          # desarrollo (requiere Node v22)
npm run build        # build producción
php artisan serve    # servidor PHP (o usar Herd)
vendor/bin/pint --dirty  # formatear PHP
```

## Arquitectura

- Una sola ruta raíz `/` → `PortfolioController@index` → Inertia `Home.vue`
- `/cv/download` → genera PDF con `spatie/laravel-pdf` desde `resources/views/pdf/cv.blade.php`
- Sin autenticación, sin base de datos

## Gestión de contenido (proyectos)

Los proyectos se gestionan con archivos Markdown en `resources/content/projects/`.
...
description: Descripción breve (usada en la tarjeta)
tags:
  - Laravel
  - Vue
featured: true
...
Descripción larga en Markdown (no usada actualmente en la UI, pero disponible).
```

### Imágenes de proyectos

- Guardar en `public/images/projects/`
- Referenciar en el frontmatter con el nombre de archivo (sin ruta)
...
- Siempre curly braces en estructuras de control.
- Nunca pasar argumentos que coincidan con el valor por defecto.
...
- El usuario hace los commits manualmente.

## Notas de diseño

- Paleta: rose `#e11d48`, purple `#a855f7`, fondo `#070712`.
- Fuentes: Space Grotesk (display), Inter (body), JetBrains Mono (mono/code).
- Efecto scanline CRT en `body::before`.
- Logo 3D con 20 `.layer` apilados con `translateZ` y `rotmio` animation.
- Canvas de partículas en `<canvas id="bg">`.
...
- `code-documenter` — Generates, formats, and validates technical documentation — including docstrings, OpenAPI/Swagger specs, JSDoc annotations, doc portals, and user guides. Use when adding docstrings to functions or classes, creating API documentation, building documentation sites, or writing tutorials and user guides. Invoke for OpenAPI/Swagger specs, JSDoc, doc portals, getting started guides.
